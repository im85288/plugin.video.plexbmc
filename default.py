...
def enforceSkinView(mode):
    ...
    Xperience1080_views = {  'List (Default)' : 50,
                       'List (Low)' : 511,
                       'List (Banners)' : 60,
                       'Poster (Classic)' : 506,
                       'Poster (Info)' : 56,
...
def shelf(server_list=None):
    ...
    for index in sorted(added_list, reverse=direction):
	    ...
	    m_fanart = getFanart(media, server_address)
            ...
            WINDOW.setProperty("Plexbmc.LatestMovie.%s.LargeThumb" % movieCount, m_fanart + qToken)
